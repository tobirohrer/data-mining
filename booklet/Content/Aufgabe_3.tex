...
In Abbildung \ref{fig:grid_search_max_depth_tree} ist  beispielhaft der Durchlauf einer \emph{Grid Search} nach der optimalen maximalen Tiefe eines Entscheidungsbaums visualisiert. Dem Beispiel, kann entnommen werden, dass eine optimale Klassifizierungsgenauigkeit auf die Testdaten für die Tiefe 5 erreicht werden kann. Bei tieferen Bäumen kommt es zu overfitting, was an der steigenden Klassifizierungsgenauigkeit der Testdaten gesehen werden kann.

\begin{figure}[ht]
	\centering
	\includegraphics[width = 0.5\textwidth]{Bilder/grid_searh_decision_tree_max_depth.png}
	\caption{Klassifizierungsgenauigkeit eines Entscheidungsbaums abhängig des Hyperparameters \emph{max\_depth}}
...
\begin{table}[h]
	\begin{tabular}{lllll}
		...
		Nach univariater \emph{Grid Search}   &  84.77\%              &  86,64\%        &        86,43\%       &    86,13\%     \\
		Nach multivariater \emph{Grid Search} &                   &          &               &         \\ \hline
	\end{tabular}
